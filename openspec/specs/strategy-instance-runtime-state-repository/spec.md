...
Define the scalar Runtime repository boundary that atomically registers, loads,
and completely replaces one in-memory strategy-instance aggregate from an
authoritative utility-derived deployment identity.
...
`list_ids_with_pending_entry_recovery() -> tuple[str, ...]`, a read-only
...
- **WHEN** some registered instances have a non-null `pending_entry_recovery`
  and others do not
- **THEN** `list_ids_with_pending_entry_recovery()` returns exactly the
...
- **THEN** `list_ids_with_pending_entry_recovery()` returns an empty tuple
...
- **THEN** `list_ids_with_pending_entry_recovery()` returns those instances'
...
#### Scenario: Keep downstream processing outside the repository
- **WHEN** `list_ids_with_pending_entry_recovery()` returns
