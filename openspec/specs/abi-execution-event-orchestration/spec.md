# abi-execution-event-orchestration Specification

## Purpose

Define `AbiExecutionEventOrchestrator` as a thin, transport-free sequencing
boundary that applies one ABI first-fill execution event to Runtime state —
keyed mutex, fresh repository load, the existing `apply_first_fill` domain
transition, and conditional save — as a second top-level writer path
alongside `StrategyRuntimeOrchestrator`, sharing its
`StrategyInstanceRuntimeStateRepository` and
`StrategyInstanceKeyedMutexRegistry`. All timestamp normalization,
candle-boundary alignment, and entry-freezing semantics remain inside
`apply_first_fill`; this orchestrator introduces no new business logic.
