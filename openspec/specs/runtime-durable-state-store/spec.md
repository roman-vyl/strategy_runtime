# runtime-durable-state-store Specification

## Purpose

Define the fsync-durable, append-only JSONL persistence for
`StrategyInstanceRuntimeState` that lets Strategy Runtime replay each
strategy instance's latest saved state after process restart or recreate.
