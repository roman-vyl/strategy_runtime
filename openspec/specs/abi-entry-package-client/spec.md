...
- **THEN** Runtime preserves the exact strings in `applied_desired_entry`, `accepted_risk_multiplier`, and `calculated_quantity`
- **AND** does not normalize them through a binary floating-point representation

#### Scenario: Reject JSON numeric decimal fields
- **WHEN** ABI returns any price, accepted multiplier, or calculated quantity as a JSON number
...
- **WHEN** HTTP `200` contains exactly `strategy_instance_id`, `trade_cycle_id`, status `entry_package_applied`, `applied_desired_entry`, `accepted_risk_multiplier`, and `calculated_quantity`
...
- **WHEN** a purported success body has a missing field, unknown field, wrong field type, invalid decimal, invalid `DesiredEntry`, or unknown status
...
- **WHEN** fake ABI emits both successes, all four public errors, mismatched identifiers, malformed payloads, timeout, network failure, redirects, and undocumented statuses
...
- **THEN** it confirms the exact method, route, nullable `desired_entry`, mandatory positive-string `risk_multiplier`, success union, closed DTOs, decimal formats, and `400`/`415`/`422`/`500` mappings
