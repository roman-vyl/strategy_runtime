...
- [x] 1.1 Add a required `position_management_orchestrator:
...
- [x] 1.2 Replace the `OpenTradeProjectedStrategyInstance` branch in
  ...
  already does.
- [x] 1.3 Remove `OpenTradeProjectionUnsupportedError` and its now-dead
  `raise` from `orchestrator/errors.py` and `orchestrator/orchestrator.py`.

## 2. Configuration

- [x] 2.1 Add `RUNTIME_ABI_POSITION_MANAGEMENT_TIMEOUT_SECONDS` to
...
- [x] 2.2 Add the new variable to `config/runtime.env.example`.

## 3. Production Composition

- [x] 3.1 In `build_application`, construct
...
- [x] 3.2 Construct `PositionManagementOrchestrator(execution_port=
  position_management_client)` and pass it into
  `StrategyRuntimeOrchestrator(...)` as
  `position_management_orchestrator=...`.
- [x] 3.3 Update the `_OutboundHttpClientLifecycle`/docstring references to
  "four outbound HTTP clients" (in `application.py`'s module and function
  docstrings) to "five".

## 4. Test Updates

- [x] 4.1 Update every `StrategyRuntimeOrchestrator(...)` construction
  site (unit orchestrator tests, production e2e test) to pass a
  `position_management_orchestrator`.
- [x] 4.2 Replace the existing `OpenTradeProjectionUnsupportedError`
...
- [x] 4.3 Update `tests/integration/committed_bar/test_production_e2e.py`
...
- [x] 4.4 Update `tests/unit/bootstrap/test_composition_root.py`'s "four
