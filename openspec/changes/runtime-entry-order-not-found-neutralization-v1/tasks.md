...
- [ ] 1.1 Add `entry_order_not_found` to the typed recovery response union with no
...
- [ ] 2.1 In the locked uncertain-APPLY branch, send exactly one existing corrective
  CANCEL when recovery returns `entry_order_not_found`, preserving the pending exact
  strategy/cycle identity.
- [ ] 2.2 Clear the marker only for exact matching formal `EntryPackageAbsent`; leave all
  state unchanged for errors, mismatches, unexpected applied results, or unknown results.
- [ ] 2.3 Add tests proving the branch never sends CREATE, never reconstructs desired
  entry, and performs at most one bounded corrective call per attempt.

## 3. Regression boundaries

- [ ] 3.1 Add tests proving uncertain removal retains its existing table and that
