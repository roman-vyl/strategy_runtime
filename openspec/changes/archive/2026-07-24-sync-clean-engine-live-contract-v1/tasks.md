...
- [x] 1.2 Replace live-entry response fields with singular `desired_entry`.
- [x] 1.3 Remove all request echo fields from both Engine response DTOs.
- [x] 1.4 Preserve calculation inputs while replacing the side-wise recipe model with `DesiredEntry`.

## 2. Scalar Router Mapping

- [x] 2.1 Remove Engine `instance_id` request mapping from both routing branches.
- [x] 2.2 Pass live-entry `desired_entry` through without side arbitration.
...
- [x] 3.1 Update live and open-trade fake Engine fixtures for singular calculation-only responses.
...
- [x] 4.1 Update Runtime System Plans and main use-case-router spec to the singular Engine contract.
