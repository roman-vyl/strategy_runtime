...
- [x] 1.1 Add `PendingEntryRecovery` (`trade_cycle_id: str` only, no timestamp) to
      `runtime/state/models.py`.
- [x] 1.2 Add `pending_entry_recovery: PendingEntryRecovery | None` to
      `StrategyInstanceRuntimeState`.
- [x] 1.3 Collapse `Replace` into `Cancel` in `runtime/entry_reconciliation/` (decision
      table, command construction, confirmation model, state applier, frozen-context
      fail-closed checks) — three-way `NoOp`/`Apply`/`Cancel`.
- [x] 1.4 Remove `Replace`-specific handling from
      `entry_reconciliation_orchestrator/orchestrator.py`.

## 2. Durable persistence

- [x] 2.1 Add `list_ids_with_pending_entry_recovery() -> tuple[str, ...]` to the
...
- [x] 2.2 Bump the durable codec to `schema_version = 2`: `pending_entry_recovery` becomes
      a required envelope key (`null` or `{trade_cycle_id}`), decoded under the existing
      exact-key-set rule.
- [x] 2.3 Decode `schema_version = 1` records as `pending_entry_recovery = None`; encode
      every new write as `schema_version = 2`. No file rewrite.

## 3. Save-before-call invariant

- [x] 3.1 In `entry_reconciliation_orchestrator/orchestrator.py`, durably save
      ...
      current-cycle id).
- [x] 3.2 On a successful confirmation, clear `pending_entry_recovery` in the same
      transition that applies the confirmed outcome.
- [x] 3.3 On an execution-port exception, leave `pending_entry_recovery` exactly as
      durably saved before the call; propagate the exception unchanged.

## 4. Orchestrator guard

- [x] 4.1 In `StrategyRuntimeOrchestrator.process(...)`, immediately after `get_or_create`
...
- [x] 5.1 Add a Runtime-side HTTP client for
      ...
      conventions.
- [x] 5.2 Decode the four `recovery_state` values (`entry_order_live`, `position_open`,
...
- [x] 5.3 Decode ABI's `422 unknown_trade_cycle_binding` as its own typed public error,
...
- [x] 5.4 Add a client method for the one corrective action (resend CANCEL for the
...
- [x] 6.1 Implement `attempt(strategy_instance_id)`: under
...
- [x] 6.2 Implement the background worker (own thread, `_State` enum, `start()`/
      `stop_once()`, fixed bounded polling interval, no adaptive/exponential backoff),
      modeled on `CommittedBarIntakeWorker`.
- [x] 6.3 Each interval, enumerate pending instances via
...
- [x] 6.4 On any response other than the four positive `recovery_state` values (transport
...
- [x] 7.1 Construct the resolver and worker in `bootstrap/application.py` alongside
      `intake_worker`.
- [x] 7.2 Start the resolver worker in `_lifespan` startup, alongside `intake_worker`.
- [x] 7.3 Stop the resolver worker before closing HTTP clients in shutdown, alongside
      `intake_worker`.

## 8. Regression coverage

- [x] 8.1 An ambiguous Apply durably preserves `pending_entry_recovery` and never loses
      the minted `trade_cycle_id` (the ETH-class regression).
- [x] 8.2 An ambiguous Cancel (including a formerly-Replace-triggered one) durably
      preserves `pending_entry_recovery`; the bar path fails closed via the guard, not via
      an uncaught `500` (the BTC-class regression).
- [x] 8.3 `decide_entry_reconciliation` never returns `Replace`; a changed desired entry
      returns `Cancel`.
- [x] 8.4 Resolver: each of the four ABI recovery states, for both the uncertain-Apply and
      uncertain-Cancel cases, including the `entry_order_live`-while-removal-intended
      resend-CANCEL path.
- [x] 8.5 Resolver: ABI's safe-error response (query failure or insufficient positive
      ...
      inconclusively."
- [x] 8.6 Codec: `schema_version = 1` records decode with `pending_entry_recovery = None`;
      `schema_version = 2` records require the key; every new write is `schema_version = 2`.
- [x] 8.7 Orchestrator guard: a committed bar for an instance with
      `pending_entry_recovery` set does not call the use-case router or Strategy Engine.
- [x] 8.8 Resolver: ABI's `422 unknown_trade_cycle_binding` leaves `pending_entry_recovery`
      untouched — it is never treated as `terminal_without_fill`.
- [x] 8.9 Resolver: an instance whose pending marker never receives a positive
...
- [x] 9.1 Run the full unit/integration/contract test suite.
- [x] 9.2 Run strict OpenSpec validation (`npm exec -- openspec validate --all --strict`).
