...
- [ ] 3.2 For any line other than the last: raise a fail-closed replay
      error on a JSON parse failure or a domain/schema validation failure.
- [ ] 3.3 For the last line only: on a JSON parse failure or a
      domain/schema validation failure, discard it (log the recovered
      truncated tail) and keep that key's last valid prior record, if any;
      otherwise apply it like any other valid record.
- [ ] 3.4 Add tests: replay of a clean file with multiple keys and
      superseding records; replay of a file with a corrupted non-last
      line (fails closed); replay of a file with a truncated last line
      (tolerated, prior record kept); replay of a file whose only line is
      truncated (tolerated, empty index).

## 4. Configuration

- [ ] 4.1 Add `state_path: Path` to `RuntimeConfig`
      (`src/strategy_runtime/config/model.py`) with the same
      non-empty-path validation as `journal_path`.
- [ ] 4.2 Read `RUNTIME_STATE_PATH` in `load_runtime_config`
      (`src/strategy_runtime/config/loader.py`), following the existing
      `RUNTIME_JOURNAL_PATH` default/validation pattern.
...
- [ ] 6.1 Add a third bind mount to `docker-compose.yml`:
      `RUNTIME_STATE_PATH` → `/runtime/state`, sourced from
      `${BBB_DATA_ROOT}/strategy-runtime/state`, writable.
- [ ] 6.2 Update the README `## Docker` section's example `docker run` and
      Compose usage to include `RUNTIME_STATE_PATH` /
      `${BBB_DATA_ROOT}/strategy-runtime/state`, alongside the existing
      specs and journal mounts.
...
- [ ] 7.2 Test that production composition reports `ready=False` on a
      corrupted state file and `ready=True` on a clean or absent one.
