...
- [ ] 1.1 Add a `FirstFillRequest` Pydantic model (or equivalent naming
...
- [ ] 1.2 Add fixed response models: a `first_fill_recorded` success model,
...
- [ ] 2.3 Route body: check `app.state.ready` and
...
- [ ] 2.4 Map exceptions raised by the callable to fixed responses:
  ...
  body.
- [ ] 2.5 Register a `RequestValidationError`-driven `400` for body
...
- [ ] 2.6 Do not add any mutex acquisition, repository call,
...
- [ ] 3.1 In `build_application` (`bootstrap/application.py`), after
...
- [ ] 3.2 Define a thin `process_first_fill(event: AbiFirstFillExecutionEvent)
...
- [ ] 4.1 The route method is exactly `PUT`.
- [ ] 4.2 The route path is exactly `/v1/strategy-instances/
  {strategy_instance_id}/trade-cycles/{trade_cycle_id}/first-fill`.
- [ ] 4.3 A valid body contains only `first_fill_at_ms`; `strategy_instance_id`
  and `trade_cycle_id` in the body (duplicating the path) are rejected as
  extra fields.
- [ ] 4.4 First successful call returns `200
  {"status":"first_fill_recorded"}`.
- [ ] 4.5 An identical retry (same path, same `first_fill_at_ms`) returns
  the identical `200 {"status":"first_fill_recorded"}` response.
- [ ] 4.6 The HTTP response is only returned after the injected application
...
- [ ] 4.7 `BackgroundTasks` is not used for this route — assert no
...
- [ ] 4.8 Each of `bool`, `float`, `string`, zero, negative, missing, and
  extra-field `first_fill_at_ms` (and any other extra top-level field)
  independently produces `400 {"status":"rejected","reason":
  "invalid_webhook"}`.
- [ ] 4.9 A callable raising `StrategyInstanceStateNotFound` produces `404
  {"status":"strategy_instance_state_not_found"}`.
- [ ] 4.10 A callable raising `FirstFillInvariantError` produces `409
  {"status":"first_fill_conflict"}`.
- [ ] 4.11 A not-ready application (`ready=False` or
  `process_first_fill=None`) produces `503 {"status":"not_ready"}` without
  invoking any callable.
- [ ] 4.12 A callable raising an unexpected exception (e.g. a bare
  `RuntimeError` standing in for a repository failure) produces `500
  {"status":"internal_error"}`.
- [ ] 4.13 A callable raising a bare `ValueError` (standing in for
...
- [ ] 4.14 The `200` response body contains no `state`, no normalized
...
- [ ] 4.15 The existing closed-bar endpoint's contract (method, path,
...
- [ ] 5.1 Given a valid path/body pair, the adapter constructs exactly one
  ...
  callable.
- [ ] 5.2 The adapter calls the injected application callable exactly once
...
- [ ] 5.4 Each known exception type (`StrategyInstanceStateNotFound`,
  ...
  status possible.
- [ ] 5.5 An unknown exception raised by a stub callable is logged via
  `app.state.logger` (assert a log call occurred) and produces exactly
  `500 {"status":"internal_error"}`.

## 6. Composition Tests

- [ ] 6.1 `AbiExecutionEventOrchestrator` is constructed exactly once by
...
- [ ] 6.5 A `ready=True` application's `app.state.process_first_fill` (or
  equivalent attribute) is a connected, non-`None` callable that invokes
  `AbiExecutionEventOrchestrator.process`.
- [ ] 6.6 A `ready=False` application never executes the first-fill use
...
- [ ] 6.7 The new wiring adds zero outbound HTTP clients — assert
  `len(app.state.outbound_http_clients) == 4` (unchanged from before this
  change) for a ready application.
- [ ] 6.8 The existing outbound-client lifecycle (construct-once,
...
- [ ] 6.9 A forced failure in constructing `AbiExecutionEventOrchestrator`
...
- [ ] 7.1 Seed a shared `InMemoryStrategyInstanceRuntimeStateRepository`
...
- [ ] 7.2 Wire that repository and a real
...
- [ ] 7.3 Send `PUT .../first-fill` with a valid `first_fill_at_ms`; assert
  `200`.
- [ ] 7.4 Inspect the repository directly: `FrozenExecutedEntryContext` is
...
- [ ] 7.5 Repeat the identical `PUT`: assert `200` again; assert the
...
- [ ] 7.6 Send a `PUT` with a different `first_fill_at_ms` for the same
  trade cycle: assert `409`; assert the stored frozen context is
  unchanged from step 7.4.

## 8. Shared-Writer Serialization Test

- [ ] 8.1 Construct one real `StrategyInstanceKeyedMutexRegistry` and one
...
- [ ] 8.2 While a closed-bar writer thread holds the mutex for
  ...
  remains held.
- [ ] 8.3 Release the closed-bar writer's hold on `instance-A`; assert the
...
- [ ] 8.4 While `instance-A`'s mutex is held, issue a first-fill `PUT` for
